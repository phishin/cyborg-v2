...
function signUpWithFacebook(event) {
  if (event) event.preventDefault()
  $firebase.signInWithPopup($firebase.auth, $firebase.facebookProvider)
      .then((result) => {
        const user = result.user
        logInUser({
          name: user.displayName,
...
async function signUpWithTwitter(e) {
  e?.preventDefault()

  // 1) Build a fresh provider so we can request email
  ...
  try {
    ...
    // 4) User object
    const user = result.user
    console.log('Twitter user:', user, accessToken, secret)

    // 5) Call your app’s login handler
    logInUser({
      name:  user.displayName,
      email: user.email,
      photo: user.photoURL,
